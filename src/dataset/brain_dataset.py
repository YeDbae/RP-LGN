...
from .abcd.load_abcd import load_data_abide, load_data_um, load_data_kki, load_data_nyu, load_data_adhd, load_data_ucla
...
class BrainDataset(InMemoryDataset):
    def __init__(self, root, name, transform=None, pre_transform: BaseTransform = None, view=0):
        ...
        assert self.name in ['UCLA', 'UM', 'KKI', 'NYU', 'ADHD', 'ABIDE', 'NYU']
    ...
    def _download(self):
        if files_exist(self.raw_paths) or self.name in ['UCLA', 'UM', 'KKI', 'ADHD', 'ABIDE', 'NYU']:  # pragma: no cover
            return
    ...
    def process(self):
        if self.name in ['ADHD', 'KKI', 'ABIDE', 'NYU', 'UM', 'UCLA']:
            #
            if self.name == 'ABIDE':
                adj, y = load_data_abide(self.raw_dir)
            elif self.name == 'ADHD':
                adj, y = load_data_adhd(self.raw_dir)
            elif self.name == 'NYU':
                adj, y = load_data_nyu(self.raw_dir)
            elif self.name == 'KKI':
                adj, y = load_data_kki(self.raw_dir)
            elif self.name == 'UM':
                adj, y = load_data_um(self.raw_dir)
            elif self.name == 'UCLA':
                adj, y = load_data_ucla(self.raw_dir)
